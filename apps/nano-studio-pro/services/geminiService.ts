...
const analyzeProductFeatures = async (
  ...
  if (!process.env.API_KEY) return { description: "", filename: "product" };
...
export const upscaleImage = async (
    ...
    return callGeminiGenerator([imgPart, { text: prompt }], resolution, onStatusUpdate);
};
...
export const processImagesWithGemini = async (
  ...
  if (presetId === PresetType.GHOST_MANNEQUIN) {
    const promises = sourceImages.map(async (img, idx) => {
        ...
        const { description, filename } = await analyzeProductFeatures(img.file, onStatusUpdate);
        ...
        const url = await callGeminiGenerator(requestParts, imageQuality, onStatusUpdate, '3:4');
        return { url, filename };
    });
  ...
  const { description, filename } = await analyzeProductFeatures(primaryImg.file, onStatusUpdate);
  ...
  const resultUrl = await callGeminiGenerator(requestParts, imageQuality, onStatusUpdate, targetAspectRatio);
